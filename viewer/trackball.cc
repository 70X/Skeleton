		     ...
		     long width, long height)
{
    ...
    /* Start up the trackball.  The trackball works by pretending that
       a ball encloses the 3D view.  You roll this pretend ball with
       the mouse.  For example, if you click on the center of the ball
       and move the mouse straight to the right, you roll the ball
       around its Y-axis.  This produces a Y-axis rotation.  You can
       click on the "edge" of the ball and roll it around in a circle
       to get a Z-axis rotation.
       
       The math behind the trackball is simple: start with a vector
       from the first mouse-click on the ball to the center of the 3D
       view.  At the same time, set the radius of the ball to be the
       smaller dimension of the 3D view.  As you drag the mouse around
       in the 3D view, a second vector is computed from the surface of
       the ball to the center.  The axis of rotation is the cross
       product of these two vectors, and the angle of rotation is the
    ...
    xxyy =
	gStartPtTrackball [0] * gStartPtTrackball[0] +
	gStartPtTrackball [1] * gStartPtTrackball [1];
...
void rollToTrackball (long x, long y, float rot [4]) // rot is output rotation angle
{
    ...
    if (fabs (gEndPtTrackball [0] - gStartPtTrackball [0]) < kTol &&
    ...
    xxyy =
	gEndPtTrackball [0] * gEndPtTrackball [0] +
	gEndPtTrackball [1] * gEndPtTrackball [1];
    ...
    rot[1] =
	gStartPtTrackball[1] * gEndPtTrackball[2] -
	gStartPtTrackball[2] * gEndPtTrackball[1];
    rot[2] =
	-gStartPtTrackball[0] * gEndPtTrackball[2] +
	gStartPtTrackball[2] * gEndPtTrackball[0];
    rot[3] =
	gStartPtTrackball[0] * gEndPtTrackball[1] -
	gStartPtTrackball[1] * gEndPtTrackball[0];
    
    // Use atan for a better angle.  If you use only cos or sin, you
    // only get half the possible angles, and you can end up with
    // rotations that flip around near the poles.
    
    // cos(a) = (s . e) / (||s|| ||e||)
    cosAng =
	gStartPtTrackball[0] * gEndPtTrackball[0] +
	gStartPtTrackball[1] * gEndPtTrackball[1] +
	gStartPtTrackball[2] * gEndPtTrackball[2]; // (s . e)
    ls = sqrt(gStartPtTrackball[0] * gStartPtTrackball[0] +
	      gStartPtTrackball[1] * gStartPtTrackball[1] +
	      gStartPtTrackball[2] * gStartPtTrackball[2]);
    ls = 1. / ls; // 1 / ||s||
    le = sqrt(gEndPtTrackball[0] * gEndPtTrackball[0] +
	      gEndPtTrackball[1] * gEndPtTrackball[1] +
	      gEndPtTrackball[2] * gEndPtTrackball[2]);
    le = 1. / le; // 1 / ||e||
    cosAng = cosAng * ls * le;
...
void addToRotationTrackball (float * dA, float * A)
{
    ...
    // Here's an excersize for the reader: it's a good idea to
    // re-normalize your quaternions every so often.  Experiment with
    // different frequencies.
    
    // An identity rotation is expressed as rotation by 0 about any
    // axis.  The "angle" term in a quaternion is really the cosine of
    // the half-angle.  So, if the cosine of the half-angle is one
    // (or, 1.0 within our tolerance), then you have an identity
    // rotation.
    ...
    // If you get here, then you have a non-identity rotation.  In
    // non-identity rotations, the cosine of the half-angle is non-0,
    // which means the sine of the angle is also non-0.  So we can
    // safely divide by sin(theta2).
